...
nonisolated final class TyKaozHost {
    ...
    nonisolated(unsafe) var onReport: ((String) -> Void)?
    nonisolated(unsafe) var onFail: ((String) -> Void)?
    /// JS-tool result delivery (`__toolResult`) for `JSToolBundle`.
    nonisolated(unsafe) var onToolResult: (([Any]) -> Void)?
    ...
    func chat(params: [Any], reply: HostReply) {
    ...
    func toolList(reply: HostReply) {
        let tools = self.tools
        Task { @MainActor in reply.resolve(Self.toolListJSON(tools)) }
    }

    func toolCall(params: [Any], reply: HostReply) {
    ...
    func memorySave(params: [Any], reply: HostReply) {
    ...
    func memoryRead(params: [Any], reply: HostReply) {
    ...
    func memoryList(reply: HostReply) {
        let memory = self.memory
        Task { @MainActor in
...
extension XSEngine {
    ...
    static func tyKaoz(host: TyKaozHost) -> XSEngine? {
...
nonisolated struct HostReply {
    let bridge: UnsafeMutableRawPointer
    let id: UInt32
    func resolve(_ json: String) { json.withCString { xsBridgeComplete(bridge, id, 1, $0) } }
    func reject(_ json: String) { json.withCString { xsBridgeComplete(bridge, id, 0, $0) } }
    func emit(_ json: String) { json.withCString { xsBridgeEmitToken(bridge, id, $0) } }
}
