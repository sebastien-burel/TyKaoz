...
enum ImageContent {
    ...
    /// can't be read.
    static func encode(_ url: URL) -> (mime: String, base64: String)? {
    ...
    static func openAIParts(for urls: [URL]) -> [[String: Any]] {
    ...
    static func anthropicBlocks(for urls: [URL]) -> [[String: Any]] {
        urls.compactMap { url in
            guard let (mime, b64) = encode(url) else { return nil }
            return [
                "type": "image",
    ...
    static func geminiParts(for urls: [URL]) -> [[String: Any]] {
        urls.compactMap { url in
            guard let (mime, b64) = encode(url) else { return nil }
