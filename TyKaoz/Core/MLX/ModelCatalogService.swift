...
final class ModelCatalogService {
    ...
    enum Source: String { case bundle, cache, network }

    private(set) var models: [CatalogModel] = []
    private(set) var updatedAt: String?
    private(set) var source: Source = .bundle
    ...
    /// `models.json`).
    var embeddings: [CatalogModel] { models.filter { $0.category == .embedding } }

    /// Chat models, in manifest order (the order they appear in
    /// `models.json`).
    var chats: [CatalogModel] { models.filter { $0.category == .chat } }

    /// Look up a catalog entry by its HuggingFace slug.
    func entry(forID id: String) -> CatalogModel? { models.first { $0.id == id } }
    ...
    static let bundledManifestJSON = """
    {
      "schema_version": 2,
